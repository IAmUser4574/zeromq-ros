...
from cmasi import GimbalPointingMode
...
class GimbalConfiguration(PayloadConfiguration.PayloadConfiguration):
    ...
    def __init__(self):
        ...
        self.ContainedSensorList = []   #uint32
    ...
    def pack(self):
        ...
        buffer.append(struct.pack(">H", len(self.ContainedSensorList) ))
        for x in self.ContainedSensorList:
            buffer.append(struct.pack(">I", x ))
    ...
    def unpack(self, buffer, _pos):
        ...
        _arraylen = struct.unpack_from(">H", buffer, _pos )[0]
        _arraylen = struct.unpack_from(">H", buffer, _pos )[0]
        self.ContainedSensorList = [None] * _arraylen
        _pos += 2
        if _arraylen > 0:
            self.ContainedSensorList = struct.unpack_from(">" + `_arraylen` + "I", buffer, _pos )
            _pos += 4 * _arraylen
        return _pos
    ...
    def get_ContainedSensorList(self):
        return self.ContainedSensorList
    ...
    def toString(self):
        ...
        buf +=    "ContainedSensorList = " + str( self.ContainedSensorList ) + "\n" 
